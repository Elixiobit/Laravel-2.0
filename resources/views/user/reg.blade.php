...
@section('content')
    ...
    <form action="{{route('user::reg::form')}}" method="post">
        ...
        <fieldset>
            ...
            <input id="name" name="user[secondName]" placeholder="Иванов" autofocus value="{{old('secondName')}}" required pattern="\S+[А-яа-я]">
            <br>
            <label for="name">Имя <em>*</em></label>
            <input id="name" name="user[firstName]"  placeholder="Иван"  pattern="\S+[А-яа-я]">
            <br>
            <label for="name">Отчество</label>
            <input id="name" name="user[middleName]" placeholder="Сидорович" pattern="\S+[А-яа-я]">
            <br>
            <label for="telephone">Телефон <em>*</em></label>
            <input id="telephone" name="user[telephone]" placeholder="+7-903-955-55-55" >
            <br>
            <label for="email">Email <em>*</em></label>
            <input id="email" name="user[email]" placeholder="email@server.com"  pattern="\S+@[a-z]+.[a-z]+">
            <br> </fieldset>
        <fieldset>
            <legend>Персональная информация</legend>
            <label for="age">Возраст<em>*</em></label>
            <input id="age" name="user[age]" type="number"  min="14" max="100">
            <br>
